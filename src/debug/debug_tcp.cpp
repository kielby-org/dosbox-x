...
/* Forward declaration — defined in debug.cpp */
...
static uint16_t configured_port = 0;
...
void DEBUG_TCP_Init(void) {
    ...
    configured_port = (uint16_t)sect->Get_int("tcp_debug_port");
    if (configured_port == 0) return;

    tcp_server = NETServerSocket::NETServerFactory(SOCKET_TYPE_TCP, configured_port);
    if (!tcp_server || !tcp_server->isopen) {
        LOG_MSG("DEBUG_TCP: Failed to listen on port %u", (unsigned)configured_port);
        delete tcp_server;
        tcp_server = nullptr;
        return;
    }

    LOG_MSG("DEBUG_TCP: Listening on port %u", (unsigned)configured_port);
}
